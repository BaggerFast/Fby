...
def catalogue_list(request):
    ...
    json_object = serializers.serialize('json', data_objects)

    json_object = json.dumps(json_object, sort_keys=True, indent=2, ensure_ascii=False)
...
def paginator(sku_codes):
    for i in range(sku_codes.len()):
        data_object = Offer.objects.get(skui)
